...
contract BamaToken is ERC20Capped {
  ...
  uint256 internal constant SEED_INVESTORS_TOKEN = (CAP * TOKEN_SIZE) * 5/100; // 5%
  ...
  // Events
  event Allocated(address indexed to, uint256 indexed amount, string indexed allocationType);
  event Minted(address indexed to, uint256 indexed amount, string indexed mintId);
  ...
  function allocateToken(address account, string calldata allocationType) external onlyOwner {
    if (allocationAccounts[allocationType] == account) revert BamaToken__HasAllocation();
  ...
  function mintToken(address account, uint256 amount, string calldata mintId) external onlyOwner {
    if ((_amountMintedBycommunity + amount) > COMMUNITY_TOKEN) revert BamaToken__NoMoreCommunityToken();
  ...
  function getAllocation(string calldata allocationType) public view returns (uint256) {
