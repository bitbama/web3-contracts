...
contract BamaStaking {
  ...
  function _updateRewardIndex(address address_) private {
    uint256 newRewardIndex = (StakerDetails[address_].totalAmtStaked /
      totalStaked) * StakerDetails[address_].totalAmtStaked;
    StakerDetails[address_].currentRewardIndex = ((newRewardIndex * 1) /
      REWARDS_PER_HOUR);
  }
  ...
  function currentRewardsEarned(
    address address_
  ) external view returns (uint256) {
    return _currentRewardsEarned(address_);
  }

  function _currentRewardsEarned(
    address address_
  ) internal view returns (uint256) {
    if (StakerDetails[msg.sender].unstaked == 1)
      return StakerDetails[address_].currentAmtEarned;

    uint256 lastUpdated = StakerDetails[address_].lastUpdatedAt;
    uint256 elapsedSeconds = (block.timestamp - lastUpdated);
    return ((StakerDetails[address_].currentRewardIndex * elapsedSeconds) /
      1 hours);
  }
  ...
  function withdraw() external {
    ...
    uint256 lastWithdrawAccessTime = StakerDetails[msg.sender]
      .lastWithdrawAccessTime;
    ...
    if (lastWithdrawAccessTime > block.timestamp)
      revert BamaStaking__NotYetWithdrawalTime();
