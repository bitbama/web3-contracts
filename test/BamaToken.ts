...
describe("BamaToken", function () {
  ...
  describe("Deployment", function () {
    ...
    it("Should mint 10,000 token to owner's account", async function () {
      const { token, owner } = await loadFixture(deployFixture)
      expect(await token.balanceOf(owner)).to.equal(ethers.parseEther("10000"))
    })
    it("Should have a total of 10,000 tokens", async function () {
      const { token } = await loadFixture(deployFixture)
      expect(await token.totalSupply()).to.equal(ethers.parseEther("10000"))
    })
    it("Should increase _amountMintedBycommunity by 10,000 tokens", async function () {
      const { token } = await loadFixture(deployFixture)
      expect(await token.getMintedBamaAmount()).to.equal(
  ...
  describe("AllocateTokens", function () {
    ...
    it("Should not allocate token if action is not initiated by owner", async function () {
      const connectedContract = bamaToken.connect(
        allocationAccounts["IDO_TOKEN"]
      )
      await expect(
        connectedContract.allocateToken(
          allocationAccounts["IDO_TOKEN"],
          "IDO_TOKEN"
        )
      ).to.be.revertedWithCustomError(bamaToken, "BamaToken__NotOwner")
    })

    for (const [key, value] of Object.entries(allocationTypes)) {
      it(`${key} account should get a total of ${
        value + ownergasFee
      } tokens`, async function () {
        await expect(
          bamaToken.allocateToken(allocationAccounts[key], key)
        ).to.changeTokenBalances(
          bamaToken,
          [bamaToken, allocationAccounts[key]],
          [0, value]
        )
        expect(await bamaToken.totalSupply()).to.equal(value + ownergasFee)
      })

      it(`${key} account should not get allocation of ${
        value + ownergasFee
      } tokens for the second time`, async function () {
        await expect(
          bamaToken.allocateToken(allocationAccounts[key], key)
        ).to.changeTokenBalances(
          bamaToken,
        ...
        await expect(
          bamaToken.allocateToken(allocationAccounts[key], key)
        ).to.revertedWithCustomError(bamaToken, "BamaToken__HasAllocation")
        expect(await bamaToken.totalSupply()).to.equal(value + ownergasFee)
      })
    }
    ...
    it("Should emit an event on allocation", async function () {
      await expect(
        bamaToken.allocateToken(allocationAccounts["IDO_TOKEN"], "IDO_TOKEN")
      )
        .to.emit(bamaToken, "Allocated")
        .withArgs(
          allocationAccounts["IDO_TOKEN"].address,
          allocationTypes["IDO_TOKEN"],
          "IDO_TOKEN"
  ...
  describe("MintTokens", function () {
    ...
    beforeEach(async function () {
      const { token, anonAccount } = await loadFixture(deployFixture)
    ...
    it("Should not mint token if action is not initiated by owner", async function () {
      ...
      await expect(
        connectedContract.mintToken(otherAccount, amount, mintId)
      ).to.be.revertedWithCustomError(bamaToken, "BamaToken__NotOwner")
    })
    ...
    it("Should not mint token if amount is greater than 10,000", async function () {
      ...
      await expect(
        bamaToken.mintToken(otherAccount, customAmt, mintId)
    ...
    it(`Account should have received a total of ${
      amount + ownergasFee
    } Bama token`, async function () {
      await expect(
        bamaToken.mintToken(otherAccount, amount, mintId)
      ).to.changeTokenBalances(
        bamaToken,
        [bamaToken, otherAccount],
        [0, amount]
      )
      expect(await bamaToken.totalSupply()).to.equal(amount + ownergasFee)
    })
