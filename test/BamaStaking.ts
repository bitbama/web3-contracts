...
import { Address, BamaStaking, BamaToken, IERC20 } from "../typechain-types"
 ...
 describe("BamaStaking", function () {
    ...
    async function deployFixture() {
        ...
        return {bamaToken, bamaStaking, tokenAddress, stakingAddress, owner, anonAccount}
    }
    ...
    describe("Deployment", function () {
        ...
        beforeEach(async function () {
           const {bamaToken, bamaStaking, tokenAddress} = await loadFixture(deployFixture)
            token = bamaToken
            staking = bamaStaking
    ...
    describe("Stake", function () {
        ...
        it(`Token Contract's total supply should equal ${mintAmount} + ${stakingTotalRewardBal}`, async function () {
            await staking.stake(amount)
            expect(await token.totalSupply()).to.eq(mintAmount + stakingTotalRewardBal)
        })
        ...
        it(`Signer Account's balance should equal ${mintAmount}`, async function () {
            await staking.stake(amount)
            expect(await token.balanceOf(signer)).to.eq(mintAmount - amount)
        })
        ...
        it("Should transfer amount", async function () {
            await expect(staking.stake(amount)).to.changeTokenBalances(token, 
                [signer, staking],
                [-amount, amount]
            )
        })
        ...
        describe("Validations", function () {
            ...
            it("Should revert if there's no Reward Token in the Pool", async function () {
                ...
                await expect(bamaStaking.stake(amount)).to.be.revertedWithCustomError(bamaStaking, "BamaStaking__NoMoreRewardToken")
            })
            ...
            it("Should revert if account has previously requested to unstake", async function () {
                const { bamaToken, bamaStaking, owner } = await loadFixture(deployFixture)
                await bamaToken.allocateToken(stakingAddr, "STAKING_TOKEN")
                await token.approve(stakingAddr, 0)
                await token.approve(stakingAddr, amount)
                ...
                await expect(bamaStaking.stake(amount)).to.be.revertedWithCustomError(bamaStaking, "BamaStaking__UnstakeInProgress")
            })
        })
        ...
        it("Should emit Staked event", async function () {
            await expect(staking.stake(amount)).to.emit(staking, "Staked").withArgs(
                signer, amount
            )
        })
    })
    ...
    //     const stakingTotalRewardBal = ethers.parseEther("60000000")
    //     const mintId = "7654hvgs2hvs"
    //     const stakeId = "7654hvgs2hvs"

    //     const customBeforeEach = async function () {
    //         const { bamaToken, bamaStaking, stakingAddress, anonAccount } = await loadFixture(deployFixture)
    //         token = bamaToken
    //         staking = bamaStaking
    //         anotherAccount = anonAccount.address
    //         stakingAddr = stakingAddress
    //         await token.allocateToken(stakingAddr, "STAKING_TOKEN")
    //         await token.mintToken(anotherAccount, mintAmount, mintId)
    //         await token.approve(stakingAddr, 0)
    //         await token.approve(stakingAddr, amount)
    //         }
    ...
    //     it(`Token Contract's total supply should equal ${mintAmount} + ${stakingTotalRewardBal}`, async function () {
    //         await staking.autoStake(anotherAccount, amount, stakeId)
    //         expect(await token.totalSupply()).to.eq(mintAmount + stakingTotalRewardBal)
    //     })
    ...
    //     it(`Signer Account's balance should equal ${mintAmount}`, async function () {
    //         await staking.autoStake(anotherAccount, amount, stakeId)
    //         expect(await token.balanceOf(anotherAccount)).to.eq(mintAmount - amount)
    //     })
    ...
    //     it("Should transfer amount", async function () {
    //         await expect(staking.autoStake(anotherAccount, amount, stakeId)).to.changeTokenBalances(token, 
    //             [anotherAccount, staking],
    //             [-amount, amount]
    //         )
    //     })
    ...
    //         await staking.autoStake(anotherAccount, amount, stakeId)
    //         const latest = await time.latest()
    //         const stakerDetails = await staking.getStakerDetails(anotherAccount)
    //         expect(stakerDetails.lastUpdatedAt).to.eq(latest)
    //     })
    //     it("Should have claimed 0 rewards", async function () {
    //         await staking.autoStake(anotherAccount, amount, stakeId)
    ...
    //             const { bamaToken, bamaStaking } = await loadFixture(deployFixture)
    //             await bamaToken.allocateToken(stakingAddr, "STAKING_TOKEN")
    //             await expect(bamaStaking.autoStake(anotherAccount, amount, stakeId)).to.be.reverted
    //         })
    //         it("Should revert if there's no Reward Token in the Pool", async function () {
    //             const { bamaStaking } = await loadFixture(deployFixture)
    //             await expect(bamaStaking.autoStake(anotherAccount, amount, stakeId)).to.be.revertedWithCustomError(bamaStaking, "BamaStaking__NoMoreRewardToken")
    //         })
    //         it("Should revert if address has insufficient balance", async function () {
    //             const { bamaToken, bamaStaking } = await loadFixture(deployFixture)
    //             const cusAmount = ethers.parseEther("100000")
    ...
    //             await token.approve(stakingAddr, 0)
    //             await token.approve(stakingAddr, cusAmount)
    //             await expect(bamaStaking.autoStake(anotherAccount, cusAmount, stakeId)).to.be.reverted
    //         })
    //         it("Should revert if account has previously requested to unstake", async function () {
    //             const { bamaToken, bamaStaking, owner, anonAccount } = await loadFixture(deployFixture)
    //             await bamaToken.allocateToken(stakingAddr, "STAKING_TOKEN")
    //             await token.approve(stakingAddr, 0)
    //             await token.approve(stakingAddr, amount)
    //             await token.mintToken(anonAccount.address, mintAmount, mintId)
    //             await bamaStaking.autoStake(anonAccount.address, amount, stakeId)
    //             await bamaStaking.unstake(amount)
    //             const stakerDetails = await bamaStaking.getStakerDetails(anotherAccount)
    //             expect(stakerDetails.unstaked).to.eq(1)
    //             await expect(bamaStaking.autoStake(anonAccount.address, amount, stakeId)).to.be.revertedWithCustomError(bamaStaking, "BamaStaking__UnstakeInProgress")
    //         })
    //     })
    ...
    //     it("Should emit Staked event", async function () {
    //         await expect(staking.autoStake(anotherAccount, amount, stakeId)).to.emit(staking, "AutoStaked").withArgs(
    //             anotherAccount, amount, stakeId
    //         )
    //     })
    // })
    ...
    describe("ClaimReward", function () {
        ...
        it("Should not change the balanceOf(signer)", async function () {
            ...
            expect(latestStakerDetails.totalAmtStaked).to.eq(stakerDetails.totalAmtStaked)
        })
        ...
        describe("Validations", function () {
            ...
            it("Should revert if signer has unstaked", async function () {
                ...
                await expect(staking.claim()).to.be.revertedWithCustomError(staking, "BamaStaking__UnstakeInProgress")
            })
        })
        ...
        describe("Events", function () {
            ...
            it("Should emit Claimed event", async function () {
                await expect(staking.claim()).to.emit(staking, "Claimed").withArgs(
                    signer, reward
                )
            })
        })
    ...
    describe("InreaseStakingPower(Compound)", function () {
        ...
        it("Should increment total amount earned", async function () {
            ...
            const latestStakerDetails = await staking.getStakerDetails(signer)
            expect(latestStakerDetails.totalAmtEarned).to.eq(stakerDetails.totalAmtEarned + reward)
        })
        it("Should increment signer's total staked balance", async function () {
            const stakerDetails = await staking.getStakerDetails(signer)
            await staking.compound()
            const latestStakerDetails = await staking.getStakerDetails(signer)
            expect(latestStakerDetails.totalAmtStaked).to.eq(stakerDetails.totalAmtStaked + reward)
        })
    ...
    describe("Unstake", function () {
        ...
        it("should not change token balances", async function () {
            await expect(staking.unstake(amount)).to.changeTokenBalances(token,
                [signer, staking],
                [0, 0]
            )
        })
        ...
        describe("Validations", function () {
            ...
            it("Should revert if signer has insufficient staked balance", async function () {
                ...
                await expect(staking.unstake(cusAmt)).to.be.revertedWithCustomError(staking, "BamaStaking__InsufficientBalance")
            })
        })
    ...
    describe("EmergencyUnstake", function () {
        ...
        it("should not change token balances", async function () {
            await expect(staking.emergencyUnstake(amount)).to.changeTokenBalances(token,
                [signer, staking],
                [0, 0]
            )
        })
        ...
        it("Should reduce balanceOf(signer)/total amount staked", async function () {
            ...
            await staking.emergencyUnstake(amount)
            const latestStakerDetails = await staking.getStakerDetails(signer)
            expect(latestStakerDetails.totalAmtStaked).to.eq(stakerDetails.totalAmtStaked - fee)
        })
        ...
        describe("Validations", function () {
            ...
            it("Should revert if signer has insufficient staked balance", async function () {
                const cusAmt = ethers.parseEther("10001")
                await expect(staking.emergencyUnstake(cusAmt)).to.be.revertedWithCustomError(staking, "BamaStaking__InsufficientBalance")
            })
        })
    ...
    describe("Withdraw", async function () {
        ...
        it("Should change token balances", async function () {
            const stakerDetails = await staking.getStakerDetails(signer)
            const finalAmount = stakerDetails.withdrawalAmount + stakerDetails.currentAmtEarned
            await expect(staking.withdraw()).to.changeTokenBalances(token,
                [signer, staking],
        ...
        it("Should decrement balanceOf(signer)/total amount staked", async function () {
            ...
            const latestStakerDetails = await staking.getStakerDetails(signer)
            expect(latestStakerDetails.totalAmtStaked).to.eq(stakerDetails.totalAmtStaked - stakerDetails.withdrawalAmount)
        })
        it("Should decrement contract's total staked balance", async function () {
            const stakerDetails = await staking.getStakerDetails(signer)
            const balance = await staking.getTotalStaked()
            await staking.withdraw()
            expect(await staking.getTotalStaked()).to.eq(balance - stakerDetails.withdrawalAmount)
        })
        it("Should update lastUpdatedAt to current block timestamp", async function () {
            await staking.withdraw()
            const timestamp = await time.latest()
        ...
        describe("Validations", function () {
            ...
            it("Should revert if signer has not initiated the unstake action earlier", async function () {
                ...
                await staking.stake(amount)
                await time.increase(60*60)
                await expect( staking.withdraw()).to.be.revertedWithCustomError(staking, "BamaStaking__HasNotUnstaked")
               })
               ...
               it("Should revert if it is not yet 21 days after untstake was initiated", async function () {
                ...
                await expect(staking.withdraw()).to.be.revertedWithCustomError(staking, "BamaStaking__NotYetWithdrawalTime")
               })
        })
        ...
        describe("Events", function () {
            ...
            it("Should emit Unstaked event", async function () {
                const stakerDetails = await staking.getStakerDetails(signer)
                const finalAmount = stakerDetails.withdrawalAmount + stakerDetails.currentAmtEarned
                await expect(staking.withdraw()).to.emit(staking, "Unstaked").withArgs(
                    signer, finalAmount
                )
            })
        })
