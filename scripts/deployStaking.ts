...
async function main() {
  ...
  const bamaStaking = await ethers.deployContract("BamaStaking", [
    `${process.env.CURRENT_BAMA_TOKEN_ADDRESS}`,
  ])
  ...
  if (
    ...
    //await verify(`${process.env.CURRENT_BAMA_TOKEN_ADDRESS}`, [])
    await verify(`${bamaStaking.target}`, [`${process.env.CURRENT_BAMA_TOKEN_ADDRESS}`])
  }
}
